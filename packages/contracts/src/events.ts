...
export const lineStatuses = ['PAID', 'ACCEPTED', 'DISPATCHED', 'DELIVERED', 'PENDING', 'REFUNDED', 'FAILED', 'CANCELLED', 'REPLACED'] as const;
...
export const eventSchemas = {
  ...
  'market.want_created': z.object({
    want_id: z.string().uuid(),
    buyer_id: z.string().uuid(),
    product_name: z.string().min(1),
    qty: z.number().int().positive(),
    unit: z.string().nullable().optional(),
    ceiling_kobo: z.number().int().positive().nullable().optional(),
    bid_count: z.number().int().nonnegative(),
  }),
  'market.negotiation_message': z.object({
    negotiation_id: z.string().uuid(),
    buyer_id: z.string().uuid(),
    seller_id: z.string().uuid(),
    message_id: z.string().uuid(),
    kind: z.enum(['BUYER_BID', 'SELLER_OFFER', 'SELLER_ACCEPT', 'BUYER_ACCEPT', 'WALK', 'CALLBACK', 'NOTE', 'REVOKE', 'END']),
    side: z.enum(['BUYER', 'SELLER']),
    qty: z.number().int().positive(),
    per_unit_kobo: z.number().int().positive().nullable().optional(),
  }),
  'notification.feed': z.object({
    notification_id: z.string().uuid(),
    user_id: z.string().uuid(),
    type: z.enum(['order', 'chat', 'market', 'deal', 'system']),
  ...
  'market.crowd_sale_created': z.object({
    ...
    product_name: z.string().min(1),
    unit: z.string().nullable().optional(),
    unit_price_kobo: z.number().int().positive(),
    qty_available: z.number().int().positive(),
    min_qty: z.number().int().positive(),
  }),
  'market.crowd_sale_closed': z.object({
    sale_id: z.string().uuid(),
    seller_id: z.string().uuid(),
  }),
...
export const SCHEMA_VERSIONS: Record<EventType, number> = {
  ...
  'market.want_created': 1,
  'market.negotiation_message': 1,
  'notification.feed': 1,
  'chat.message': 1,
  'market.crowd_sale_created': 1,
  'market.crowd_sale_closed': 1,
