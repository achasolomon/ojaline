...
import { sendChatMessage, getChatMessages, getUserConversations, type ChatMessage } from '../lib/api';
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex shrink-0 items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-base font-semibold text-text">{partyName || 'Chat'}</h1>
          <p className="truncate text-[11px] font-medium text-textSecondary">Ojaline inbox</p>
        </div>
        <button
          type="button"
          onClick={() => alert('VoIP calling coming soon. For now, use in-app chat to coordinate delivery.')}
          className="flex shrink-0 items-center gap-1.5 bg-primary-light text-primary text-xs font-semibold px-3 py-1.5 rounded-lg border-none cursor-pointer"
        >
      ...
      {warnings.length > 0 && (
        <div className="shrink-0 bg-[#fef3cd] border-b border-[#ffc107] px-4 py-2">
          {warnings.map((w, i) => (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        {messages.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <div className="mb-3 grid h-12 w-12 place-items-center rounded-full bg-primary-light text-primary">
