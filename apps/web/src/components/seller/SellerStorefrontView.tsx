import type { Offer, Seller } from '../../lib/api';
  ...
  return (
    <>
      ...
      <div className="mb-6 overflow-hidden rounded-2xl border border-border bg-white">
        {seller.banner_url ? (
          <div
            className="h-20 bg-cover bg-center lg:h-28"
            style={{ backgroundImage: `url(${mediaUrl(seller.banner_url)})` }}
            role="img"
            aria-label={`${seller.full_name}'s storefront banner`}
          />
        ) : (
            ...
            <div className="w-[104px] shrink-0 lg:w-[140px]">
              <div className="aspect-square rounded-full bg-white p-1.5 shadow-[0_4px_14px_rgba(0,0,0,0.08)]">
                {seller.profile_photo_url ? (
                  <img
                    src={mediaUrl(seller.profile_photo_url) ?? ''}
                ...
                <div className="mt-2 flex max-h-8 flex-wrap gap-1.5 overflow-hidden">
                  {markets.map((m) =>
                    onOpenMarket ? (
                      ...
                      <span
                        key={m.id}
                        className="flex items-center gap-1 rounded-full border border-border bg-surface px-2.5 py-0.5 text-[10px] font-semibold text-textSecondary"
                      >
                        <Icon name="map" size={11} /> {m.name}
                      </span>
                    ),
      ...
      <h2 className="mb-4 text-lg font-black text-text">
        ...
        <span className="ml-2 text-sm font-normal text-textSecondary">({products.length})</span>
      </h2>
        ...
        <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-3 lg:gap-3.5">
          {products.map((product) => (
            <OfferCard
              key={product.id}
              offer={product}
              onClick={onOpenProduct ? (o) => onOpenProduct(o) : undefined}
            />
          ))}
        </div>
      )}
